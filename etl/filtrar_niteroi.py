...
def process():
    ...
    trans_path = os.path.join(base_dir, "transacoes_bruto.csv")
    trans_out_path = os.path.join(base_dir, "transacoes_bruto_niteroi.csv")
    ...
    codigos_niteroi = set(df_niteroi['cod_bairro'].dropna())
    bairros_niteroi = set(df_niteroi['nome_bairro'].apply(normalize_string))
    
    print(f"Encontrados {len(codigos_niteroi)} códigos de bairro e {len(bairros_niteroi)} nomes de bairros para Niterói.")
    
    print("Processando transações... (isso pode levar alguns segundos)")
    chunksize = 100000
    first_chunk = True
    total_transacoes = 0
    
    for chunk in pd.read_csv(trans_path, chunksize=chunksize, dtype=str):
        # A regra é: Niterói em PELO MENOS UM dos campos (origem ou destino)
        mask = chunk['cod_bairro_origem'].isin(codigos_niteroi) | chunk['cod_bairro_destino'].isin(codigos_niteroi)
        chunk_niteroi = chunk[mask]
        total_transacoes += len(chunk_niteroi)
        
        if first_chunk:
            chunk_niteroi.to_csv(trans_out_path, index=False, mode='w', encoding='utf-8')
            first_chunk = False
        else:
            chunk_niteroi.to_csv(trans_out_path, index=False, mode='a', header=False, encoding='utf-8')
            
    print(f"Total de {total_transacoes} transações salvas em: {trans_out_path}")
